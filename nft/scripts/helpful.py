...
from brownie import (accounts, config, network, LinkToken,
...
def get_account(index=None, id=None):
    if index:
        print(f"account get with index {index}")
        return accounts[index]
    if network.show_active() in LOCAL_BLOCKCHAIN_ENVIRONMENTS:
        print(f"account get in local env")
        return accounts[0]
    if id:
        print(f"account get with {id}")
